  ...
  -- Neopyter for live JupyterLab sync and execution from Neovim
  {
    ...
    config = function()
      local function patch_neopyter_notebook_parser()
        ...
        if not ok_notebook or type(notebook) ~= "table" or notebook._neotex_parser_patched then
          return
        end
        ...
        notebook._neotex_parser_patched = true
      end

      local function patch_neopyter_direct_disconnect()
        local ok_direct, direct_client = pcall(require, "neopyter.rpc.direct")
        if not ok_direct or type(direct_client) ~= "table" or direct_client._neotex_disconnect_patched then
          return
        end
        ...
        direct_client._neotex_disconnect_patched = true
      end
      ...
      local augroup = vim.api.nvim_create_augroup("neotex-neopyter-keymaps", { clear = true })
